...
Data <-data0[,c('sp','x','y')]; names(Data) <- c('sp', 'gx', 'gy')
head(Data)
#######
plotdim <- c(500, 300)
#
...
SAR=function(data=Data, nx=20, plotdim=c(500,300),
             process="Hom.Po",
...
#   thomas.estK.my(X, startpar = c(kappa = 1, sigma2 = 1), maxit=1e+4, lambda = NULL, 
#                            q = 1/4, p = 2, rmin = NULL, rmax = NULL) 
#  thomas.estK.my(X=K,  startpar = c(sigma2=K$r[chose]/4, kappa=1/K$border[chose]),
#                       lambda=lambda, maxit=1e+5 )
...
#     
#     
     result <- mincontrast.my(observed=K, theoretical=theoret,
                              startpar,maxit=1e+4,
                              ctrl = list(q = 1/4, 
